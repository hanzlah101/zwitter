"use client";

import { FC, useState } from "react";
import axios from "axios";
import { MdOutlineVideoLibrary } from "react-icons/md";
import { RxCross2 } from "react-icons/rx";
import EmojiPicker from "emoji-picker-react";
import Avatar from "./Avatar";
import { useRouter } from "next/navigation";
import { SafeUser } from "../types";
import { toast } from "react-hot-toast";
import useZweetModal from "../hooks/useZweetModal";

interface FormProps {
  currentUser: SafeUser | null;
}

const Form: FC<FormProps> = ({ currentUser }) => {
  const [caption, setCaption] = useState("");
  const [showPicker, setShowPicker] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  const [images, setImages] = useState<any>([]);
  const [selectedIndex, setSelectedIndex] = useState(0);
  const [percentage, setPercentage] = useState(0);

  const router = useRouter();
  const zweetModal = useZweetModal();

  function handleKeyDown(e: any) {
    e.target.style.height = "inherit";
    e.target.style.height = `${e.target.scrollHeight}px`;
    e.target.style.height = `${Math.min(e.target.scrollHeight, 200)}px`;
  }

  const handleChange = (event: any) => {
    const files = event.target.files;
    const newFiles: any = [];

    for (let i = 0; i < files.length; i++) {
      const file = files[i];
      const reader = new FileReader();

      reader.onload = () => {
        const fileContent = reader.result;
        const fileType = file.type;

        if (fileType.startsWith("image/") || fileType.startsWith("video/")) {
          newFiles.push({
            content: fileContent,
            type: fileType,
            name: file.name,
          });
        }
      };

      reader.readAsDataURL(file);
    }

    setTimeout(() => {
      setImages([...images, ...newFiles]);
    }, 1000);
  };

  const handleRemoveImage = (i: number) => {
    const updatedImages = [...images];
    updatedImages.splice(i, 1);
    setImages(updatedImages);
    if (selectedIndex === i && i !== 0) {
      setSelectedIndex(i - 1);
    }
  };

  const handleSubmit = async (e: any) => {
    e.preventDefault();
    setIsLoading(true);
    setPercentage(0);

    try {
      await axios.post(
        "/api/post",
        {
          caption,
          images: images.map((img: any) => img.content),
        },
        {
          headers: { "Content-Type": "multipart/json" },
          onUploadProgress: (event: any) => {
            const percentCompleted = Math.round(
              (event.loaded * 100) / event.total
            );

            setPercentage(percentCompleted);
          },
        }
      );

      toast.success("Post created successfully!");
      zweetModal.isOpen && zweetModal.onClose();
      setCaption("");
      setImages([]);
      router.refresh();
    } catch (error: any) {
      console.log(error);
      toast.error(error.response.data.message);
    } finally {
      setIsLoading(false);
      setPercentage(0);
    }
  };

  return (
    <form
      onSubmit={(e) => handleSubmit(e)}
      className={`pb-4 relative border-gray-200 ${
        isLoading && "animate-pulse"
      }`}
    >
      <div className="flex flex-shrink-0 p-4 pb-0">
        <div className="w-12 flex items-top">
          <Avatar src={currentUser?.avatar} alt={currentUser?.username} />
        </div>

        <div className="w-full p-2">
          <textarea
            id="captionInput"
            readOnly={isLoading}
            value={caption}
            onChange={(e) => setCaption(e.target.value)}
            onKeyDown={(e) => handleKeyDown(e)}
            className="w-full placeholder-gray-400 inline-block bg-transparent border-0 focus:outline-none resize-none"
            placeholder="What's happening?"
          />
        </div>
      </div>

      <div className="w-full flex items-top p-2 text-white pl-14">
        <div
          title="Photo"
          className="cursor-pointer text-blue-400 hover:bg-blue-50 rounded-full p-2"
        >
          <label className="cursor-pointer" htmlFor="image">
            <input
              readOnly={isLoading}
              accept="image/png,image/jpg,image/jpeg, image/webp"
              type="file"
              className="hidden"
              id="image"
              onChange={(e) => handleChange(e)}
              multiple
            />

            <svg viewBox="0 0 24 24" className="w-5 h-5" fill="currentColor">
              <g>
                <path d="M19.75 2H4.25C3.01 2 2 3.01 2 4.25v15.5C2 20.99 3.01 22 4.25 22h15.5c1.24 0 2.25-1.01 2.25-2.25V4.25C22 3.01 20.99 2 19.75 2zM4.25 3.5h15.5c.413 0 .75.337.75.75v9.676l-3.858-3.858c-.14-.14-.33-.22-.53-.22h-.003c-.2 0-.393.08-.532.224l-4.317 4.384-1.813-1.806c-.14-.14-.33-.22-.53-.22-.193-.03-.395.08-.535.227L3.5 17.642V4.25c0-.413.337-.75.75-.75zm-.744 16.28l5.418-5.534 6.282 6.254H4.25c-.402 0-.727-.322-.744-.72zm16.244.72h-2.42l-5.007-4.987 3.792-3.85 4.385 4.384v3.703c0 .413-.337.75-.75.75z"></path>
                <circle cx="8.868" cy="8.309" r="1.542"></circle>
              </g>
            </svg>
          </label>
        </div>

        <div
          title="Video"
          className="cursor-pointer text-xl text-blue-400 hover:bg-blue-50 rounded-full p-2"
        >
          <label className="cursor-pointer" htmlFor="video">
            <input
              readOnly={isLoading}
              accept="video/*"
              type="file"
              className="hidden"
              id="video"
              onChange={(e) => handleChange(e)}
              multiple
            />
            <MdOutlineVideoLibrary />
          </label>
        </div>

        <div
          title="Gifs"
          className="cursor-pointer text-blue-400 hover:bg-blue-50 rounded-full p-2"
        >
          <label className="cursor-pointer" htmlFor="gif">
            <input
              readOnly={isLoading}
              accept="image/gif"
              type="file"
              className="hidden"
              id="gif"
              onChange={(e) => handleChange(e)}
              multiple
            />
            <svg viewBox="0 0 24 24" className="w-5 h-5" fill="currentColor">
              <g>
                <path d="M19 10.5V8.8h-4.4v6.4h1.7v-2h2v-1.7h-2v-1H19zm-7.3-1.7h1.7v6.4h-1.7V8.8zm-3.6 1.6c.4 0 .9.2 1.2.5l1.2-1C9.9 9.2 9 8.8 8.1 8.8c-1.8 0-3.2 1.4-3.2 3.2s1.4 3.2 3.2 3.2c1 0 1.8-.4 2.4-1.1v-2.5H7.7v1.2h1.2v.6c-.2.1-.5.2-.8.2-.9 0-1.6-.7-1.6-1.6 0-.8.7-1.6 1.6-1.6z"></path>
                <path d="M20.5 2.02h-17c-1.24 0-2.25 1.007-2.25 2.247v15.507c0 1.238 1.01 2.246 2.25 2.246h17c1.24 0 2.25-1.008 2.25-2.246V4.267c0-1.24-1.01-2.247-2.25-2.247zm.75 17.754c0 .41-.336.746-.75.746h-17c-.414 0-.75-.336-.75-.746V4.267c0-.412.336-.747.75-.747h17c.414 0 .75.335.75.747v15.507z"></path>
              </g>
            </svg>
          </label>
        </div>

        <div
          title="Emoji"
          aria-disabled={isLoading}
          className="text-blue-400 hover:bg-blue-50 rounded-full p-2 cursor-pointer"
          onClick={() => setShowPicker(!showPicker)}
        >
          <svg viewBox="0 0 24 24" className="w-5 h-5" fill="currentColor">
            <g>
              <path d="M12 22.75C6.072 22.75 1.25 17.928 1.25 12S6.072 1.25 12 1.25 22.75 6.072 22.75 12 17.928 22.75 12 22.75zm0-20C6.9 2.75 2.75 6.9 2.75 12S6.9 21.25 12 21.25s9.25-4.15 9.25-9.25S17.1 2.75 12 2.75z"></path>
              <path d="M12 17.115c-1.892 0-3.633-.95-4.656-2.544-.224-.348-.123-.81.226-1.035.348-.226.812-.124 1.036.226.747 1.162 2.016 1.855 3.395 1.855s2.648-.693 3.396-1.854c.224-.35.688-.45 1.036-.225.35.224.45.688.226 1.036-1.025 1.594-2.766 2.545-4.658 2.545z"></path>
              <circle cx="14.738" cy="9.458" r="1.478"></circle>
              <circle cx="9.262" cy="9.458" r="1.478"></circle>
            </g>
          </svg>
        </div>

        <button
          disabled={caption === "" || isLoading}
          className="ml-auto text-sm disabled:bg-primaryBlue/60 disabled:cursor-not-allowed text-white bg-blue-400 px-4 rounded-full font-semibold hover:bg-blue-500"
        >
          Zweet
        </button>
      </div>

      {showPicker && (
        <div className="absolute z-20">
          <EmojiPicker
            onEmojiClick={(e: any) => {
              setShowPicker(false);
              document.getElementById("captionInput")?.focus();
              setCaption(caption + e.emoji);
            }}
          />
        </div>
      )}

      <div className={`mx-3 ${isLoading && "animate-pulse"}`}>
        {images.length > 0 && (
          <div className="w-full relative overflow-y-hidden">
            <div className="w-full relative mt-4 overflow-y-hidden max-h-[540px] h-full">
              <button
                disabled={isLoading}
                onClick={() => handleRemoveImage(selectedIndex)}
                className="absolute disabled:cursor-not-allowed disabled:opacity-70 top-2 z-10 left-2 text-xl text-white bg-gray-400/40 border border-white rounded-full p-1 cursor-pointer"
              >
                <RxCross2 />
              </button>
              {images[selectedIndex]?.type?.startsWith("image/") && (
                <img
                  src={images[selectedIndex].content}
                  alt={images[selectedIndex].name}
                  className="object-cover rounded-2xl w-full max-h-[540px] h-full"
                />
              )}

              {images[selectedIndex]?.type?.startsWith("video/") && (
                <video
                  controls
                  src={images[selectedIndex].content}
                  autoPlay
                  loop
                  className="object-cover rounded-2xl w-full max-h-[540px] h-full"
                />
              )}
            </div>

            <div className="mt-3 flex items-center gap-2 overflow-x-auto hide-scrollbar">
              {images.length > 1 &&
                images.map((img: any, i: number) => {
                  return (
                    <div className="relative" key={i}>
                      <button
                        disabled={isLoading}
                        onClick={() => handleRemoveImage(i)}
                        className="absolute disabled:cursor-not-allowed disabled:opacity-70 top-1 z-10 left-1 text-base text-black bg-white/75 border border-black rounded-full p-0.5 cursor-pointer"
                      >
                        <RxCross2 />
                      </button>
                      {img.type.startsWith("image/") && (
                        <div className="w-40 h-28">
                          <img
                            src={img.content}
                            alt={img.name}
                            className={`rounded-xl h-full w-full object-cover cursor-pointer ${
                              i === selectedIndex
                                ? "opacity-100 border border-black"
                                : "opacity-50 border-none"
                            }`}
                            onClick={() => setSelectedIndex(i)}
                          />
                        </div>
                      )}

                      {img.type.startsWith("video/") && (
                        <div className="w-40 h-28">
                          <video
                            src={img.content}
                            autoPlay
                            muted
                            loop
                            className={`rounded-xl h-full w-full object-cover cursor-pointer ${
                              i === selectedIndex
                                ? "opacity-100 border border-black"
                                : "opacity-50 border-none"
                            }`}
                            onClick={() => setSelectedIndex(i)}
                          />
                        </div>
                      )}
                    </div>
                  );
                })}
            </div>
          </div>
        )}
      </div>
    </form>
  );
};

export default Form;
